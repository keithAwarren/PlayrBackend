...
router.get('/lyrics', requiresAuth, async (req, res) => {
  ...
  try {
    ...
    const cachedLyrics = await queryRecord('SELECT lyrics FROM lyrics WHERE track_name = ? AND artist_name = ?', [trackName, artistName]);
    ...
    await insertRecord('lyrics', ['track_name', 'artist_name', 'lyrics'], [trackName, artistName, lyrics]);
